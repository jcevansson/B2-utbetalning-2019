...
Public Class FormTransaktion
    ...
    Public Sub refill_utbetalning_treeview()
        ...
        'toolTip1.SetToolTip(TransTreeView, "Fyller fönstret...")
        ...
        End Try
        'toolTip1.SetToolTip(TransTreeView, "")
        Status_Update()
        Cursor = Cursors.Default
    End Sub
    ...
    Private Sub Status_Update()
        ...
        SparaSomExcelToolStripMenuItem.Enabled = False
        ...
        Else
            ...
            If row("STATUS").ToString.ToUpper.Trim = "ASSIGNED" Or row("STATUS").ToString.ToUpper.Trim = "NEW" Then
                ...
                If row("Transaktion_typ").ToString.ToUpper.Trim = "UTLAND" Then
                    ...
                    SparaSomExcelToolStripMenuItem.Enabled = True
            ...
            ElseIf row("STATUS").ToString.ToUpper.Trim = "CLOSED" Then
                RapporterMI.Enabled = True
                If row("Transaktion_typ").ToString.ToUpper.Trim = "UTLAND" Then SparaSomExcelToolStripMenuItem.Enabled = True
                Rapport_Acconto.Enabled = True
        ...
        lblAntalRader.Text = TransTreeView.GetTotalRows().ToString("N0") + " rader"
        'lblTotalAmount.Text = Math.Round(TransTreeView.GetTotalAmount()).ToString("N0") + " kr"

        lblTotalAmount.Text = TransTreeView.Graph_Pad_Left(Math.Round(TransTreeView.Agg_Total_Amount("bg_netto_calc")).ToString("N0") & " kr.", 20, Me.Font)
        'Console.WriteLine("Efter första summeringarna :" & s.ElapsedMilliseconds.ToString)
        lblBrutto.Text = TransTreeView.Graph_Pad_Left(Math.Round(TransTreeView.Agg_Total_Amount("bg_belopp")).ToString("N0") & " kr.", 20, Me.Font)
    ...
    Function Get_where_string() As String
        ...
        If cb_Filter_Betalning_Typ.CheckedItems.Count < cb_Filter_Betalning_Typ.Items.Count Then
            ...
            If cb_Filter_Betalning_Typ.CheckedItems.Contains("TVCopy") = True Then StringIn &= IIf(FirstIn, "", " OR ") & " rtrim(Betalning_typ) like 'TVCopy'" : FirstIn = False
            If cb_Filter_Betalning_Typ.CheckedItems.Contains("TVCopy-Foto") = True Then StringIn &= IIf(FirstIn, "", " OR ") & " rtrim(Betalning_typ) like 'TVCopy-Foto'" : FirstIn = False
            If cb_Filter_Betalning_Typ.CheckedItems.Contains("TV4-Copy") = True Then StringIn &= IIf(FirstIn, "", " OR ") & " rtrim(Betalning_typ) like 'TV4-Copy'" : FirstIn = False
            If cb_Filter_Betalning_Typ.CheckedItems.Contains("TV4-Copy-Foto") = True Then StringIn &= IIf(FirstIn, "", " OR ") & " rtrim(Betalning_typ) like 'TV4-Copy-Foto'" : FirstIn = False
        ...
        If Filter_Konto_Typ.CheckedItems.Count < Filter_Konto_Typ.Items.Count Then
            If Filter_Konto_Typ.CheckedItems.Contains("AVI") = True Then
                StringIn &= IIf(FirstIn, "", " OR ") & "BP_KONTO_TYP = 'AVI'"
                FirstIn = False
            End If

            If Filter_Konto_Typ.CheckedItems.Contains("Bankgiro") = True Then
                StringIn &= IIf(FirstIn, "", " OR ") & "BP_KONTO_TYP = 'Bankgiro'"
                FirstIn = False
            End If

            If Filter_Konto_Typ.CheckedItems.Contains("Bankkonto") = True Then
                StringIn &= IIf(FirstIn, "", " OR ") & "BP_KONTO_TYP ='Bankkonto'"
                FirstIn = False
            End If

            If Filter_Konto_Typ.CheckedItems.Contains("Plusgiro") Then
                StringIn &= IIf(FirstIn, "", " OR ") & "BP_KONTO_TYP = 'Plusgiro'"
                FirstIn = False
            End If
        ...
        If tb_filter_kontonr.Text.Trim.Trim.Length > 1 Then f &= " and konto_nummer like '%" & tb_filter_kontonr.Text.Trim.Trim.Replace("*", "%").Replace(" ", "") & "%' "
        If tb_filter_clearing.Text.Trim.Trim.Length > 1 Then f &= " and clearing_nummer like '%" & tb_filter_clearing.Text.Trim.Trim.Replace("*", "%").Replace(" ", "") & "%' "
                ...
                While reader.Read
                    ...
                    If reader("BP_KONTO_TYP").ToString.ToUpper.Trim = "AVI" Then
                        ...
                        If reader("konto_nummer").ToString.Length > 0 Then
                            str.Append(reader("konto_nummer").ToString.Trim.PadLeft(10, "0").Substring(0, 10))
                        Else
                    ...
                    ElseIf reader("BP_KONTO_TYP").ToString.ToUpper.Trim = "PLUSGIRO" Then
                        '-------------------------------------------------------------
                        ' Skriver kontantbetalningsuppdrag Girering PLusgiro (TK 54)
                        '-------------------------------------------------------------
                        str.Length = 0
                        str.Append("54")
                        If reader("konto_nummer").ToString.Length > 0 Then
                        ...
                        '-------------------------------------------------------------
                        ' Skriver Informationspost PLusGiro (TK 65) med kontonummer
                        '-------------------------------------------------------------
                        ' str = str & Replace(objStr.AddR(strBankGiroNr, 10), " ", "0") & _
                        'objStr.AddL(strBetalningsSpec, 50) & objStr.AddR(" ", 18)
                        'For i As Integer = 0 To Int((My.Settings.BG_Meddelande.Trim.Length - 1) / 50)
                        str.Length = 0
                        str.Append("65").Append(RensaTkn(reader("konto_nummer").ToString).Trim.PadLeft(10, "0").Substring(0, 10)) 'ingen checksiffra
                        str.Append(My.Settings.BG_Meddelande.Trim.Substring(0, Math.Min(35, My.Settings.BG_Meddelande.Trim.Length)).PadRight(35))
                        str.Append(" ".PadLeft(33))
                        '                         str.Append("-" & str.ToString.Length & "-")
                        outString.AppendLine(str.ToString.ToUpper)
    ...
    Private Sub SparaSomExcelToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles SparaSomExcelToolStripMenuItem.Click
        'EXEL_Dir My.Settings.
        Dim xlWorkBook As Object 'Excel.Workbook
        Dim xlWorkSheet As Object 'Excel.Worksheet
        Dim formatRange As Object 'Excel.Range
        FBD_Excelark.SelectedPath = My.Settings.EXCEL_Dir
        Dim xlApp As Object = CreateObject("Excel.application") ' Excel.Application = New Microsoft.Office.Interop.Excel.Application()

        If xlApp Is Nothing Then
            MessageBox.Show("Excel i rätt version(16) inte installerad på den här maskinen")
            Return
        End If
        Cursor = Cursors.WaitCursor

        If (FBD_Excelark.ShowDialog = Windows.Forms.DialogResult.OK) Then
            If FBD_Excelark.SelectedPath <> My.Settings.EXCEL_Dir Then My.Settings.EXCEL_Dir = FBD_Excelark.SelectedPath
            Cursor = Cursors.WaitCursor
            Dim row As DataRow = CType(TransTreeView.SelectedNode.Tag, DataRow)
            Dim sqlConn As SqlConnection = New SqlConnection(My.Settings.DBconnectionString)
            sqlConn.Open()
            If row("Transaktion_typ").ToString.ToUpper.Trim = "UTLAND" Then
                ...
                Cursor = Cursors.WaitCursor
                selectCommand.CommandType = CommandType.StoredProcedure
                selectCommand.CommandText = "[sp_betalning_transaktion_Excelfil]"
                selectCommand.Parameters.Clear()
                parmID = selectCommand.Parameters.Add("betalning_transaktion_id", SqlDbType.Int)
                parmID.Value = row("bt_id")
                '@fil_namn as varchar(100)='X.XX',
                ...
                Dim Organisation_name As String = ""
                Dim typ As String = ""
                Dim Filename As String = ""
                Dim misValue As Object = System.Reflection.Missing.Value
                Dim sumBrutto As Double = 0.0
                Dim sumAvdrag As Double = 0.0
                Cursor = Cursors.WaitCursor
                If reader.Read Then
                    index += 1

                    Organisation_name = reader("mottagare_efternamn").ToString.Trim : typ = reader("typ").ToString.Trim
                    Filename = reader("mottagare_efternamn").ToString.Trim + "_" + reader("typ").ToString.Trim + "_" + (reader("transaktion_dag")).toShortDateString + "_" + (Now()).ToShortDateString + ".xls"
                    'SKAPA EXCELARK
                    ...
                    formatRange = xlWorkSheet.Cells(1, 1)
                    formatRange.EntireRow.Font.Bold = True

                    formatRange = xlWorkSheet.Cells(2, 1)
                    formatRange.EntireRow.Font.Bold = True
                    formatRange = xlWorkSheet.Range("a2", "i2")
                    formatRange.Interior.Color = Color.FromArgb(204, 255, 204)


                    formatRange = xlWorkSheet.Cells(4, 1)
                    formatRange.EntireRow.Font.Bold = True
                    formatRange = xlWorkSheet.Range("a4", "i5")
                    formatRange.Interior.Color = Color.FromArgb(204, 255, 204)

                    '  formatRange = xlWorkSheet.Cells(1, 1)
                    ' formatRange.EntireColumn.ShrinkToFit = True
                    xlWorkSheet.Range("A:A").ColumnWidth = 12
                    xlWorkSheet.Range("B:B").ColumnWidth = 18
                    xlWorkSheet.Range("C:C").ColumnWidth = 18
                    xlWorkSheet.Range("D:D").ColumnWidth = 20
                    xlWorkSheet.Range("E:E").ColumnWidth = 20
                    xlWorkSheet.Range("F:F").ColumnWidth = 15

                    xlWorkSheet.Range("G:G").ColumnWidth = 12
                    xlWorkSheet.Range("H:H").ColumnWidth = 12
                    xlWorkSheet.Range("I:I").ColumnWidth = 12
                    xlWorkSheet.Range("G:I").NumberFormat = "#,##0.00 _k_r"



                    xlWorkSheet.Cells(1, 1) = Organisation_name.ToString.Trim
                    xlWorkSheet.Cells(2, 1) = "IPI number"
                    xlWorkSheet.Cells(2, 2) = "Surename"
                    xlWorkSheet.Cells(2, 3) = "Firstname"
                    xlWorkSheet.Cells(2, 4) = "User"
                    xlWorkSheet.Cells(2, 5) = "Titel"
                    xlWorkSheet.Cells(2, 6) = "Rights"
                    xlWorkSheet.Cells(2, 7) = "Gross SEK"
                    xlWorkSheet.Cells(2, 8) = "Fee SEK"
                    xlWorkSheet.Cells(2, 9) = "Net SEK"
                    xlWorkSheet.Cells(4, 1) = reader("Typ")
                    ...
                    Do
                        index += 1
                        If reader("mottagare_efternamn").ToString.Trim <> Organisation_name Or reader("Typ").ToString.Trim <> typ Then
                            Cursor = Cursors.WaitCursor
                            'Spara excelark och skapa nytt
                            formatRange = xlWorkSheet.Range("a" + (rad + 4).ToString, "i" + (rad + 5).ToString)
                            formatRange.Interior.Color = Color.FromArgb(204, 255, 204)

                            formatRange = xlWorkSheet.Cells(rad + 8, 1)
                            formatRange.EntireRow.Font.Bold = True
                            xlWorkSheet.Cells(rad + 8, 7) = sumBrutto
                            xlWorkSheet.Cells(rad + 8, 8) = sumAvdrag
                            xlWorkSheet.Cells(rad + 8, 9) = sumBrutto - sumAvdrag

                            xlWorkBook.SaveAs(FBD_Excelark.SelectedPath + "\" + Filename, 56, misValue, misValue, misValue, misValue, _
                             3, misValue, misValue, misValue, misValue, misValue)
                            ...
                            formatRange = xlWorkSheet.Cells(2, 1)
                            formatRange.EntireRow.Font.Bold = True
                            formatRange = xlWorkSheet.Range("a2", "h2")
                            formatRange.Interior.Color = Color.FromArgb(204, 255, 204)
                            ...
                            xlWorkSheet.Range("F:F").ColumnWidth = 10
                            xlWorkSheet.Range("G:G").ColumnWidth = 10
                            xlWorkSheet.Range("H:H").ColumnWidth = 10
                            xlWorkSheet.Range("I:I").ColumnWidth = 10
                            xlWorkSheet.Range("G:I").NumberFormat = "#,##0.00 _k_r"
                        ...
                        End If
                        rad = rad + 1
                        xlWorkSheet.Cells(rad, 1) = reader("IPI")
                        xlWorkSheet.Cells(rad, 2) = reader("enamn")
                        xlWorkSheet.Cells(rad, 3) = reader("fnamn")
                        xlWorkSheet.Cells(rad, 4) = reader("firmanamn")
                        xlWorkSheet.Cells(rad, 5) = reader("titel")
                        xlWorkSheet.Cells(rad, 6) = reader("Typ")
                        xlWorkSheet.Cells(rad, 7) = reader("BG_Belopp") : sumBrutto = sumBrutto + reader("BG_Belopp")
                        xlWorkSheet.Cells(rad, 8) = reader("BG_Avdrag") : sumAvdrag = sumAvdrag + reader("BG_Avdrag")
                        xlWorkSheet.Cells(rad, 9) = reader("BG_Belopp") - reader("BG_Avdrag")
